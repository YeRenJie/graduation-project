...
<asp:Content ID="Content4" ContentPlaceHolderID="cpNav" runat="server">
    ...
    <li class="ind"><a href="/myadmin/notes/note.aspx"><i class=""></i>收支管理</a></li>
...
<asp:Content ID="Content3" ContentPlaceHolderID="cpMain" runat="server">
    <div class="noteAdd">
        <p>在这里，您可以增加收支管理，查看消费详情，还可以根据关键词查询记录。双击便可修改记录。</p>
        <p>
            <span><b>搜索关键字：</b> </span>
            <input type="text" id="kw" name="kw" value="<% if (!string.IsNullOrEmpty(Request.QueryString["kw"])) { Response.Write(Request.QueryString["kw"]); } %>" />&nbsp; <span id="search"><a title="搜索">搜索</a></span>&nbsp;&nbsp;&nbsp;<span style="color: #999">注:可以根据消费名称，消费地址，消费方式查询</span>
        </p>
        <p>
            <span><b>时间段查询：</b>从
                <input class="Wdate js" type="text" onclick="WdatePicker()" name="gBegain" id="gBegain" style="width: 90px; padding: 0 2px; border: 1px solid #d9d9d9;" value="<% if (arr == null) { Response.Write(firstDate.ToString("yyyy-MM-dd")); } else { Response.Write(arr[0]); } %>" />
                到
                <input class="Wdate js" type="text" onclick="WdatePicker()" name="gEnd" id="gEnd" style="width: 90px; padding: 0 2px; border: 1px solid #d9d9d9;" value="<% if (arr == null) { Response.Write(DateTime.Now.ToString("yyyy-MM-dd")); } else { Response.Write(arr[1]); } %>" /></span>&nbsp;
            <span id="searchjs"><a title="搜索">搜索</a></span>&nbsp;&nbsp;&nbsp;<span style="color: #999">注:可以根据某一时间段查询收支管理</span>
        </p>
        <table id="adsn">
            <tr>
                ...
                <td><b>支付方式：</b><select name="gBuyWay" id="gBuyWay"><option value="现金">现金</option>
    ...
    <div class="noteList">
            ...
            <asp:Repeater ID="rp" runat="server">
                <HeaderTemplate>
                    <tr class="tb">
                        ...
                        <th style="width: 105px">支出 =<b style="color: #f00"><%=countPay %></b></th>
                    </tr>
                </HeaderTemplate>
                <ItemTemplate>
                    <tr>
        ...
        <div class="fPage"><span>当前导航页 <b><%=cp %></b> / <b><%=pageCount %></b>，共 <b><%=totalSize %></b> 条记录 </span>&nbsp;&nbsp;&nbsp;&nbsp;<a href="?page=1&<%=us %>" title="首页">首页</a><a href="<%=prevHtml %>" title="上一页">上一页</a><a href="<%=nextHtml %>" title="下一页">下一页</a><a href="<%="?page="+pageCount+us %>" title="尾页">尾页</a></div>
    </div>
</asp:Content>
